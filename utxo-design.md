...
- [One-shot Tokens](#one-shot-tokens)
  * [GovernanceState token](#governancestate-token)
  * [AdminState token](#adminstate-token)
- [Governance Minting Policy](#governance-minting-policy)
  * [UserStakeDetail token](#userstakedetail-token)
  * [LastEpochScriptState token (1 per epoch)](#lastepochscriptstate-token-1-per-epoch)
  * [CanLockStake Token](#canlockstake-token)
- [Governance Validator](#governance-validator)
  * [DepositAct](#depositact)
  * [WithdrawAct](#withdrawact)
  * [ProvideRewardAct](#providerewardact)
  * [ClaimRewardAct / DistributeRewardAct](#claimrewardact--distributerewardact)
  * [FinishRewardsForEpochAct](#finishrewardsforepochact)
  * [RegisterProposalFactoryAct](#registerproposalfactoryact)
  * [LockForVotingAct](#lockforvotingact)
  * [TriggerEpochAct](#triggerepochact)
- [Proposal Minting Policy (example)](#proposal-minting-policy-example)
  * [ProposalState Token](#proposalstate-token)
- [ProposalFactory (Example)](#proposalfactory-example)
  * [CreateProposalAct](#createproposalact)
- [IndividualProposal (Example)](#individualproposal-example)
  * [VoteAct](#voteact)
- [Admin Minting Policy](#admin-minting-policy)
  * [VaultRecord tokens (1 per vault)](#vaultrecord-tokens-1-per-vault)
  * [VaultState token (1 per vault)](#vaultstate-token-1-per-vault)
- [Admin Validator](#admin-validator)
  * [InitVaultAct](#initvaultact)
  * [UpdateAdminStateAct](#updateadminstateact)
  * [InitiateUpgradeAct](#initiateupgradeact)
- [Vault Validator](#vault-validator)
  * [AddCollateralAct](#addcollateralact)
  * [RemoveCollateralAct](#removecollateralact)
  * [AddBorrowAct](#addborrowact)
  * [RepayBorrowAct](#repayborrowact)
  * [UpdateStabilityFeeAct](#updatestabilityfeeact)
  * [UpdateVaultStateAct](#updatevaultstateact)
  * [LiquidationCallAct](#liquidationcallact)
- [Oracle Minting Policy](#oracle-minting-policy)
  * [PriceTracking Token](#pricetracking-token)
- [Oracle Validator](#oracle-validator)
  * [UpdateOracleAct](#updateoracleact)
- [Buffer Monetary Policy](#buffer-monetary-policy)
  * [BufferState token (1x per supported collateral currency)](#bufferstate-token-1x-per-supported-collateral-currency)
- [Buffer Validator](#buffer-validator)
  * [LiquidationPurchaseAct](#liquidationpurchaseact)
  * [DebtPurchaseAct](#debtpurchaseact)
  * [SurplusPurchaseAct](#surpluspurchaseact)
- [Treasury Minting Policy](#treasury-minting-policy)
  * [UpgradeContract Token Minting (2 in existence for all time)](#upgradecontract-token-minting-2-in-existence-for-all-time)
  * [$dUSD Token](#dusd-token)
  * [CanMintOrSpend Token Minting](#canmintorspend-token-minting)
  * [CostCenterState token (1x for each CostCenter)](#costcenterstate-token-1x-for-each-costcenter)
...
Validator Datum and Redeemer types are documented using a state/action scheme, the datum will always reflect a State Token held by the validator script, which should be verified to match before trusting any explicit Datum argument.
...
parameters: 
```
GovStateMintingParams { governanceScriptAddress :: Address
...
| RegisterProposalFactoryAct { scriptAddress :: Address }
...
- user may have included a `UserStakeDetail` from the Governance validator script matching their `address`, 
  ...
  in both cases, the resulting Minting Policy should be sent (back) to the Governance Validator script address.
...
- UserStakeDetail Token (MINTED?) -> Governance Validator script (increment `UserStakeDetail.amountStaked` by `DepositAct.amount` OR mint fresh with `DepositAct.amount` as the initial value (other values should be initialized to defaults).
...
Validation Rules:
- if UserStakeDetail does not exist for the user's address, fail.
- if amount specified in WithdrawAct.amount is greater than the balance of the `UserStakeDetail.amount`, fail
- transaction must be signed by `UserStakeDetail.address`
- if `UserStakeDetail.lockedUntil` is a future PosixTime, fail.
- if the UserStakeDetail token is being burned, then the user `WithdrawAct.amount` must equal `UserStakeDetail.amountStaked` and vice versa, the token must be burned they are equal.
...
Validation Rules:
- VaultState must not have expired
...
### UpdateVaultStateAct
Purpose: Update Vault State to reflect changes made in the AdminState.

*known issue: we may need large-scale parallelized duplication to make this scale correctly), or to institute a `SignedMessage` trick like we do with oracle.*
...
- AdminState token -> Admin Validator Script
...
### PriceTracking Token 
  ...
  Token Type: Signed Token used for authentic pricing information from a centralized source, this token must carry the signed data to prevent bottlenecks on decentralized datum duplication.
