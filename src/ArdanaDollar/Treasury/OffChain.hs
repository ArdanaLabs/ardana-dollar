...
import Plutus.Contract.Request qualified as ContractReq (currentTime)
...
treasuryInst t =
  ...
  where
    wrap = Scripts.wrapValidator @TreasuryState @TreasuryAction
...
findTreasury ::
  ...
  (AsContractError e) =>
  Treasury ->
  Contract w s e (Maybe (Contexts.TxOutRef, Ledger.ChainIndexTxOut, TreasuryState))
findTreasury treasury = do
  ...
  where
    ...
    tokenAC = treasury'stateTokenAssetClass treasury
...
startTreasury vh peggedCurrency ownerAuthTokenAC permissionTokenAC = do
  pk <- ownPubKey
  currentTime <- ContractReq.currentTime
  ...
  case utxos of
    ...
    (oref, o) : _ -> do
      let stParams = treasuryStateTokenParams oref
          treasury =
            Treasury
              ...
              , treasury'stateTokenAssetClass = treasuryStateTokenAssetClass stParams
              }
          (tokenLookup, tokenTx, tokenValue) = treasuryStateTokenMintingConstraints stParams

          ts =
            TreasuryState
              { certTokenStart = [nat| 0 |]
              , certTokenBase = [nat| 0 |]
              , certTokenExpiration = currentTime + 60000
              , refreshInterval = 60000
              , validRangeSize = 60000
              , timestamp = currentTime
              , ownerAuthToken = ownerAuthTokenAC
              , dUSDPermissionToken = permissionTokenAC
              , extra = PlutusTx.toBuiltinData ()
              }

          treasuryValue = tokenValue <> mempty <> Value.assetClassValue danaAssetClass 10
          adminValue = mempty
      let lookups =
            tokenLookup
          ...
          tx =
            tokenTx
              <> Constraints.mustPayToTheScript ts treasuryValue
      ...
      logInfo @String $ printf "initialized Treasury %s" (show ts)
...
--     Just (oref, o, ts) -> do
--       let ts' = ts {costCenters = UniqueMap.alter centerName (Just depositValue <>) (costCenters ts)}
...
--               <> Constraints.unspentOutputs (Map.singleton oref o)
--           tx =
--             Constraints.mustPayToTheScript ts' scriptValue
...
--     Nothing -> logError @String "no treasury utxo at the script address"
--     Just (_, _, ts) ->
--       sendBus . Vector.fromList . UniqueMap.toList $ costCenters ts
