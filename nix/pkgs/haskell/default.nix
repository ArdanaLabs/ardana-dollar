{ lib, sources, agdaWithStdlib, stdenv, haskell-nix, buildPackages
, writeShellScript, checkMaterialization, gitignore-nix, R, rPackages, z3
...
let
  ...
  baseProject = { deferPluginErrors }:
    import ./haskell.nix {
      #inherit lib;
      inherit stdenv haskell-nix buildPackages R rPackages z3;
  ...
  projectPackagesAllHaddock =
    haskell-nix.haskellLib.selectProjectPackages projectAllHaddock.hsPkgs;
...
in rec {
  inherit index-state compiler-nix-name project projectAllHaddock
    projectPackages projectPackagesAllHaddock packages;
