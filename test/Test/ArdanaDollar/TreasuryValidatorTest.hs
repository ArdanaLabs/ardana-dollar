...
--   treasuryState <- forAll ArdanaGen.treasuryState

--   let sToken = Value.assetClassValue (treasury'stateTokenAssetClass treasury) 1
--       uToken = Value.assetClassValue (treasury'upgradeTokenSymbol treasury) 1

--   validation <- validationBuilder treasuryState ncHash sToken uToken
...
--       where
--         go ::
--           (TreasuryState -> TreasuryAction -> Ledger.ScriptContext -> Bool) ->
...
--       Ledger.Value ->
--       PropertyT IO (WithScript 'ForSpending ())
--     validationBuilder ts ncHash sToken uToken = do
--       let newDatum = ts {currentContract = ncHash}
--           oldContractST = contractScriptType (currentContract ts)
...
--           testData = SpendingTest ts (InitiateUpgrade $ NewContract ncHash) (sToken <> uToken)
...
--             SpendingTest ts (InitiateUpgrade $ NewContract ncHash) (sToken <> uToken)
...
--             SpendingTest ts (InitiateUpgrade $ NewContract ncHash) sToken
...
--     validationBuilder ts ncHash sToken uToken = do
--       ncHash' <- forAll PlutusGen.validatorHash
--       i <- forAll $ Gen.filter (/= auctionDanaAmount ts) Gen.integer
--       cc <- forAll $ Gen.filter (/= costCenters ts) ArdanaGen.treasuryCostCenters
...
--       return $ do
--         shouldn'tValidate "does not change datum" testData (ctxBuilder ts)
--         shouldn'tValidate "saves wrong validator hash" testData $
--           ctxBuilder (ts {currentContract = ncHash'})
