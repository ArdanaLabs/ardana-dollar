...
import Prelude (String,show,error,IO,Enum,Eq,Ord,Bounded,Show,minBound,maxBound,putStrLn)
...
insertPointInSpace coverage p s@SpaceExploration { .. } =
  case constraintViolations p of
    ...
    _ -> if length detritus < coverage
            then s { detritus = p:detritus }
            else s

spaceExploration :: forall (m :: Type -> Type). MonadGen m
                 => Integer -> StateT SpaceExploration m ()
