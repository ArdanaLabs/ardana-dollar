...
genConstraintList :: forall (m :: Type -> Type). MonadGen m
                  => m [Constraint]
genConstraintList = Gen.subsequence [minBound .. maxBound]

genTestParameters :: forall (m :: Type -> Type). MonadGen m
...
-- if if if if if lol
genSpenderParams :: forall (m :: Type -> Type). MonadGen m
                 => Integer -> ReaderT [Constraint] m SpenderParams
...
genInputUTXOParams :: forall (m :: Type -> Type). MonadGen m
...
genOutputUTXOParams :: forall (m :: Type -> Type). MonadGen m
                   => Integer -> (Integer,Integer) -> ReaderT [Constraint] m StateUTXOParams
genOutputUTXOParams w ts = do
  stok <- genStateToken
  d <- genOutputDatumParameters w ts
  return $ StateUTXOParams stok d

genKnownWalletIdx :: forall (m :: Type -> Type). MonadGen m
                  => m Integer
genKnownWalletIdx = Gen.integral (Range.linear 1 9)


genWalletIdxOtherThan :: forall (m :: Type -> Type). MonadGen m
                      => Integer -> m Integer
genWalletIdxOtherThan w = Gen.element $ filter (/= w) [1 .. 9]

genOutputTimeStamp :: forall (m :: Type -> Type). MonadGen m
                   => (Integer,Integer) -> ReaderT [Constraint] m Integer
...
genInputTimeStamp :: forall (m :: Type -> Type). MonadGen m
                   => ReaderT [Constraint] m Integer
genInputTimeStamp = Gen.integral (Range.linear 0 100_000)

genTimeRange :: forall (m :: Type -> Type). MonadGen m
...
genOutputDatumParameters :: forall (m :: Type -> Type). MonadGen m
                       => Integer -> (Integer,Integer) -> ReaderT [Constraint]  m TestDatumParameters
genOutputDatumParameters w ts = do
  violations <- ask
  walletIdx <- if OutputDatumSignedByOwner `elem` violations
...
genInputDatumParameters :: forall (m :: Type -> Type). MonadGen m
...
genStateTokenCurrencySymbol :: forall (m :: Type -> Type). MonadGen m
...
genStateTokenTokenName :: forall (m :: Type -> Type). MonadGen m
...
genStateTokenAmount :: forall (m :: Type -> Type). MonadGen m
                    => ReaderT [Constraint] m Integer
...
genStateToken :: forall (m :: Type -> Type). MonadGen m
